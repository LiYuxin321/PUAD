...
class CrossAttention(nn.Module):
    def __init__(
            self, args, dim, n_outputs=None, num_heads=8, attention_dropout=0.1, projection_dropout=0.0
    ):
        super().__init__()
        self.args = args
    ...
    def forward(self, x, y):
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale / self.args.softmax_t
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
...
class OTmodel(nn.Module):
    ...
    def __init__(
            self,
            args, device, model_state='train'
    ):
        super().__init__()
        self.state = model_state
        self.args = args
        self.device = device
        self.mus = nn.Parameter(torch.randn(1, self.args.n_concepts, self.args.d_model_concept),
                                requires_grad=True)
        self.mus = nn.init.kaiming_normal_(self.mus)

        self.mus = nn.Parameter(self.mus)

        self.mus_self = nn.Parameter(torch.randn(1, self.args.n_privately_concepts, self.args.d_model_concept),
                                     requires_grad=True)
        self.mus_self = nn.init.kaiming_normal_(self.mus_self)

        self.mus_self = nn.Parameter(self.mus_self)
    ...
    def init_mus_self(self):
        kmeans = KMeans(n_clusters=self.args.n_privately_concepts, random_state=0).fit(
            self.mus.detach().cpu().numpy().squeeze())
        centers = kmeans.cluster_centers_
        self.mus_self = nn.Parameter(torch.from_numpy(np.array(centers)).to(self.device).unsqueeze(0))
    ...
    def forward(self, x):
        if np.isnan(np.sum(x.detach().cpu().numpy())):
            a = 0
        x_temp_debug = x

        x = self.scaleEachUnitaryDatas(x)
        x = self.centerDatas(x)

        if np.isnan(np.sum(x.detach().cpu().numpy())):
            a = 0

        if self.state == 'train':
            p_xj, loss = self.getProbas_all(x)
            # p_xj = (p_xj / p_xj.sum(-1).unsqueeze(-1))
            p_xj = F.softmax(p_xj / 0.1, dim=-1)
            embadding = torch.matmul(p_xj.detach(), self.mus)
        else:
            p_xj, loss = self.getProbas_all_test(x)
            # p_xj = (p_xj / p_xj.sum(-1).unsqueeze(-1))
            p_xj = F.softmax(p_xj / 0.1, dim=-1)
            embadding = torch.matmul(p_xj.detach(), torch.cat((self.mus, self.mus_self), dim=1))

        loss_mse = self.mse(embadding, x)
        # return embadding, loss, loss_mse, p_xj
    ...
    def compute_optimal_transport(self, M, r, c, epsilon=1e-6):
        ...
        while torch.max(torch.abs(u - P.sum(2))) > epsilon:
            ...
            if iters == maxiters:
                break
            iters = iters + 1
        # return P, torch.sum(P.detach() * M) / len(P)

        return P, (P.detach() * M).sum(-1).sum(-1)

    def getProbas_all(self, ndatas):
        # compute squared dist to centroids [n_runs][n_samples][n_ways]
        # dist = (ndatas.unsqueeze(2) - self.mus.unsqueeze(1)).norm(dim=3).pow(2)
        ndatas = ndatas.detach()

        all_mus = self.mus
        dist = (ndatas.unsqueeze(2) - all_mus.unsqueeze(1)).pow(3).norm(dim=3)
        p_xj = torch.zeros_like(dist)
        r = torch.ones(len(ndatas), self.args.T)
        c = torch.ones(len(ndatas), self.args.n_concepts) * 10

        # r = r / r.sum(1).unsqueeze(1)
        # c = c / c.sum(1).unsqueeze(1)*500

        p_xj, loss = self.compute_optimal_transport(dist, r, c, epsilon=1e-6)

        return p_xj, loss

    def getProbas_all_test(self, ndatas):
        # compute squared dist to centroids [n_runs][n_samples][n_ways]
        # dist = (ndatas.unsqueeze(2) - self.mus.unsqueeze(1)).norm(dim=3).pow(2)
        ndatas = ndatas.detach()
        all_mus = torch.cat((self.mus, self.mus_self), dim=1)

        dist = (ndatas.unsqueeze(2) - all_mus.unsqueeze(1)).pow(3).norm(dim=3)
        p_xj = torch.zeros_like(dist)
        r = torch.ones(len(ndatas), self.args.T)
        c = torch.ones(len(ndatas), self.args.n_privately_concepts + self.args.n_concepts) * 10

...
def vis_fea_three(a, b, c):
    ...
    for j in range(len(a)):
        plt.scatter(x_center[j, 0], x_center[j, 1], marker='o', c='r')

    for i in range(len(b) + 1):
        plt.scatter(x_center[len(a) + i, 0], x_center[len(a) + i, 1], marker='^', c='b')

    for k in range(len(c)):
        plt.scatter(x_center[len(a) + len(b) + k, 0], x_center[len(b) + len(a) + k, 1], marker='*', c='g')

    plt.savefig('./resoult/6.jpg')
    ...
    def train_model(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, dataitem in enumerate(self.trainloader):
                ...
                out, _, otloss, loss_mse, KL_ma, KL_z, other = self.model(data)
                ...
                theta_plot_train.append(other['theta'][:, -1, :])
                # kl_mas.append(0)
                # kl_zs.append(0)
                # loss_metas.append(loss_meta.item())
            theta_plot_train = torch.cat(theta_plot_train).detach().cpu().numpy().astype(float)
            ...
            if epoch % 1 == 0:
                ...
                for i_test_data_loder in range(len(self.test_loader)):
                    ...
                    theta_plot.append(torch.cat(theta_i).cpu().numpy().astype(float))
...
def run():
    ...
    # data set
    parser.add_argument('--data_set', type=str, default='SMD')  # 'SMD' or 'MSL'
    # GPU option
    parser.add_argument('--gpu_id', type=int, default=0)
    # Dataset options
    parser.add_argument('--Train_dataset_path', type=str, default='./SMD/data_processed2/')
    parser.add_argument('--Test_dataset_path', type=str, default='./SMD/data_processed/')
    ...
    parser.add_argument('--learning_rate_finetune', type=float, default=0.00008)
    parser.add_argument('--epochs', type=int, default=350)

    parser.add_argument('--level', type=float, default=0.006)  # 1.3
    parser.add_argument('--level2', type=float, default=0.002)
    ...
    if args.data_set == 'SMD':
        ...
        args.Train_dataset_path = './MSL/train/'
        args.Test_dataset_path = './MSL/test/'
        args.input_c = 55
        args.output_c = 55
        args.n = 55
